...
import { createProject, createTask, createMilestone, updateMilestone } from "../server/db/index.js";

function insertCommit(db: Database.Database, sha: string, subject: string, authorEmail: string | null, authoredAt: string, linkedTaskId?: string) {
  db.prepare(
    `INSERT OR IGNORE INTO commits (sha, subject, author_email, authored_at, linked_task_id, ingested_at)
     VALUES (?, ?, ?, ?, ?, datetime('now'))`
  ).run(sha, subject, authorEmail, authoredAt, linkedTaskId ?? null);
}
...
describe("unlinked-commit detector", () => {
  it("emits one match per unlinked commit in the last 7 days", () => {
    insertCommit(db, "recent", "drive-by fix", "a@b", new Date().toISOString());
    insertCommit(db, "old", "old fix", "a@b", new Date(Date.now() - 30 * 86_400_000).toISOString());
  ...
  it("ignores linked commits", () => {
    ...
    insertCommit(db, "linked", "feat: with id", null, new Date().toISOString(), t.id);
    const matches = runDetectors(db, { minScore: 0 }).filter((m) => m.detectorId === "unlinked-commit");
