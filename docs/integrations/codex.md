...
**Option A: Streamable HTTP (recommended)**: requires the Vibe Dash server running, in `~/.codex/config.toml` (global, all Codex sessions):
...
**Option C: Per-run `--mcp-server` flag** (either transport)

```bash
codex --mcp-server '{"name":"vibe-dash","url":"http://localhost:3001/mcp"}' \
  "your prompt here"
```
...
- **stdio path issues** (Option B): use an absolute path; `~/` is not always expanded by the CLI
