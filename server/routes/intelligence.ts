...
export function intelligenceRoutes(db: Database.Database, _broadcast: BroadcastFn): Router {
  const router = Router();
  const intelligenceLimiter = makeReadLimiter(120);

  // GET /api/intelligence/status — whether AI is configured.
  router.get("/api/intelligence/status", intelligenceLimiter, (_req, res) => {
  ...
  router.get("/api/intelligence/digest", intelligenceLimiter, async (req, res) => {
    if (!isAiConfigured()) {
  ...
  router.post("/api/intelligence/query", intelligenceLimiter, async (req, res) => {
    if (!isAiConfigured()) {
      res.status(503).json({ error: "AI not configured — set ANTHROPIC_API_KEY" });
      return;
    }
  ...
  router.get("/api/intelligence/anomalies", intelligenceLimiter, (req, res) => {
  ...
  router.get("/api/intelligence/should-send", intelligenceLimiter, (req, res) => {
